...
// This function acts as Riches and Minerals of Spira. Please see the main file for documentation.

void RichesAndMineralsOfSpira (int* pDay, int *pWallet, int* pDebt, int* pPhoenixDown, int* pElixirEssence, int* pPlatinumIngot, int* pGoldenMateria, int* pScarletite, int* pAdamantite, int* pDarkMatter, int* pTrapezohedron)
{
	int dAction, dBuy, dSell, dAmount, dSellBack;
	
	int dPricePD = getPrice(1200,500), dStockPD = getStock(35,11), dChancePD = getRandomizer(10,1), dSellPD; 
	int dPriceEE = getPrice(2100,1500), dStockEE = getStock(25,12), dChanceEE = getRandomizer(10,1), dSellEE; 
	int dPricePI = getPrice(7000,5000), dStockPI = getStock(19,10), dChancePI = getRandomizer(10,1), dSellPI;
	int dPriceGM = getPrice(5500,3500), dStockGM = getStock(16,7), dChanceGM = getRandomizer(10,1), dSellGM;
	int dPriceSL = getPrice(12000, 8000), dStockSL = getStock(14,6), dChanceSL = getRandomizer(10,1), dSellSL;
	
	int dPriceAD = getPrice(20000,9500), dStockAD = getStock(30,15), dSellAD;
	int dPriceDM = getPrice(50000,22000), dStockDM = getStock(30,15), dSellDM;
	
	int dPriceTH = getPrice(90000,60000), dStockTH = getStock(4,2), dChanceTH = getRandomizer(10,3), dSellTH;
	...
	do
	{
		...
		if (*pAdamantite == 0)
		{
			printf("Shopkeeper: It looks like you don't have Adamantites to sell.\n");
			system("pause");
		}
		else
		{
		...
		if (*pDarkMatter == 0)
		{
			printf("Shopkeeper: It looks like you don't have Dark Matters to sell.\n");
			system("pause");
		}
		else
		{
			printf("Shopkeeper: How many Dark Matters do you want to sell?\n");
