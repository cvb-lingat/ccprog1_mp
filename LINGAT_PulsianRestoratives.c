...
// This function acts as Pulsian Restoratives. Please see the main file for documentation.

void PulsianRestoratives (int* pDay, int *pWallet, int* pDebt, int* pPhoenixDown, int* pElixirEssence, int* pPlatinumIngot, int* pGoldenMateria, int* pScarletite, int* pAdamantite, int* pDarkMatter, int* pTrapezohedron)
{
	int dAction, dBuy, dSell, dAmount, dSellBack;
	int dPricePD = getPrice(800,300), dStockPD = getStock(45,28), dSellPD;
	int dPriceEE = getPrice(1700,1000), dStockEE = getStock(35,19), dSellEE;
	
	int dPricePI = getPrice(7000,5000), dStockPI = getStock(16,7), dChancePI = getRandomizer(10,1), dSellPI;
	int dPriceGM = getPrice(5500,3500), dStockGM = getStock(25,12), dChanceGM = getRandomizer(10,1), dSellGM;
	int dPriceSL = getPrice(12000, 8000), dStockSL = getStock(12,8), dChanceSL = getRandomizer(10,1), dSellSL;
	int dPriceAD = getPrice(30000, 15000), dStockAD = getStock(10,5), dChanceAD = getRandomizer(10,1), dSellAD;
	...
	if (dAction == 1)
	{
		...
		do
		{
		scanf("%d", &dBuy); // work on the input validation
		}
		while (dBuy < 1 || dBuy > 8);
	...
	if (dAction == 2)
	{
		...
		do
		{
		scanf("%d", &dSell); // work on the input validation
		}
