...
// This function acts as Gaiain Gratitudes. Please see the main file for documentation.

void GaiainGratitudes (int* pDay, int *pWallet, int* pDebt, int* pPhoenixDown, int* pElixirEssence, int* pPlatinumIngot, int* pGoldenMateria, int* pScarletite, int* pAdamantite, int* pDarkMatter, int* pTrapezohedron)
{
	int dAction, dBuy, dSell, dAmount, dSellBack;
	
	int dPricePD = getPrice(1200,500), dStockPD = getStock(28,11), dChancePD = getRandomizer(10,1), dSellPD; 
	int dPriceEE = getPrice(2100,1500), dStockEE = getStock(20,7), dChanceEE = getRandomizer(10,1), dSellEE; 
	int dPricePI = getPrice(7000,5000), dStockPI = getStock(14,5), dChancePI = getRandomizer(10,1), dSellPI;
	int dPriceGM = getPrice(5500,3500), dStockGM = getStock(15,8), dChanceGM = getRandomizer(10,1), dSellGM;
	
	int dPriceSL = getPrice(10000,5000), dStockSL = getStock(30,15), dSellSL;
	
	int dPriceAD = getPrice(30000, 15000), dStockAD = getStock(8,4), dChanceAD = getRandomizer(10,1), dSellAD;
	int dPriceDM = getPrice(70000, 40000), dStockDM = getStock(5,3), dChanceDM = getRandomizer(10,1), dSellDM;
